...
import random, flashmquiz

import cliui, cpq, pau, mne
...
def learn(flags = ()):
    ...
    while remaining: #isn't empty
        ...
        if 'FLIP_QA' in flags:
            card.reverse()
        a = raw_input(card[0] +'? Answer (optional; q! to end) -> ')
        ...
        if a == 'q!':
            ...
            del remaining[x]
        elif 'ASKRIGHT' not in flags or raw_input(
            'Was it right? (y/n) -> '
        ).upper() == 'Y':
            del remaining[x]
...
def delete_menu():
    if cwq.set: #isn't empty
        ...
        show_cards(True)
        cardindices = raw_input('Delete which card(s)? -> ').split(',')
...
def main():
    ...
    stay = cwq != False #open_quiz_file returns False if user wants 
